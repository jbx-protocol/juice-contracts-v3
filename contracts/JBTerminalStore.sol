...
contract JBTerminalStore is ReentrancyGuard, IJBTerminalStore {
    ...
    /// @param _accountingContexts The accounting contexts of tokens whose balances should contribute to the surplus
    /// being calculated.
        ...
        returns (uint256)
    {
        ...
        return _surplusFrom(
            _terminal, _projectId, _accountingContexts, RULESETS.currentOf(_projectId), _decimals, _currency
        );
    }
    ...
    /// @param _useTotalSurplus A flag indicating whether the surplus used in the calculation should be summed from all
    /// of the project's terminals. If false, surplus should be limited to the amount in the specified `_terminal`.
    /// @return The amount of surplus tokens that can be reclaimed by redeeming `_tokenCount` tokens as a fixed point
        ...
        returns (uint256)
    {
        ...
        uint256 _totalSupply =
            IJBController(address(DIRECTORY.controllerOf(_projectId))).totalTokenSupplyWithReservedTokensOf(_projectId);
    ...
    /// @param _tokenCount The number of tokens to make the calculation with, as a fixed point number with 18 decimals.
    /// @param _totalSupply The total number of tokens to make the calculation with, as a fixed point number with 18
    /// decimals.
    ...
    /// @param _amount The amount of tokens being paid. Includes the token being paid, their value, the number of
    /// decimals included, and the currency of the amount.
    /// @param _projectId The ID of the project being paid.
    /// @param _beneficiary The address that should be the beneficiary of anything the payment yields (including project
        ...
        returns (JBRuleset memory ruleset, uint256 tokenCount, JBPayHookPayload[] memory hookPayloads)
    {
        ...
        // number of decimals as the `_amount`.
        uint256 _weightRatio = _amount.currency == ruleset.baseCurrency()
            ? 10 ** _amount.decimals
            : PRICES.pricePerUnitOf(_projectId, _amount.currency, ruleset.baseCurrency(), _amount.decimals);
    ...
    /// @param _balanceTokenContexts The token contexts whose balances should contribute to the surplus being reclaimed
    /// from.
        ...
        returns (JBRuleset memory ruleset, uint256 reclaimAmount, JBRedeemHookPayload[] memory hookPayloads)
    {
        ...
        uint256 _currentSurplus = ruleset.useTotalSurplusForRedemptions()
            ? _currentTotalSurplusOf(_projectId, _accountingContext.decimals, _accountingContext.currency)
            : _surplusFrom(
                msg.sender,
                _projectId,
        ...
        if (_currentSurplus != 0) {
            ...
            reclaimAmount = _reclaimableSurplusDuring(ruleset, _tokenCount, _totalSupply, _currentSurplus);
        }

        // Create the struct that describes the amount being reclaimed.
        JBTokenAmount memory _reclaimedTokenAmount = JBTokenAmount(
            _accountingContext.token, reclaimAmount, _accountingContext.decimals, _accountingContext.currency
        );
        ...
        if (ruleset.useDataHookForRedeem() && ruleset.dataHook() != address(0)) {
            // Yet another scoped section prevents stack too deep. `_state`  only used within scope.
            {
                ...
                (reclaimAmount, hookPayloads) = IJBRulesetDataHook(ruleset.dataHook()).redeemParams(_data);
            }
        }
        ...
        returns (JBRuleset memory ruleset, uint256 amountPaidOut)
    {
        ...
        uint256 _newUsedPayoutLimitOf = usedPayoutLimitOf[msg.sender][_projectId][_accountingContext.token][ruleset
            .cycleNumber][_currency] + _amount;

        // Amount must be within what is still available to pay out.
        uint256 _payoutLimit = IJBController(address(DIRECTORY.controllerOf(_projectId))).fundAccessLimits()
            .payoutLimitOf(_projectId, ruleset.id, msg.sender, _accountingContext.token, _currency);
        ...
        amountPaidOut = (_currency == _accountingContext.currency)
            ...
            : mulDiv(
                ...
                PRICES.pricePerUnitOf(_projectId, _currency, _accountingContext.currency, _MAX_FIXED_POINT_FIDELITY)
            );
    ...
    /// @param _accountingContext The accounting context of the token whose balances should contribute to the surplus
        ...
        returns (JBRuleset memory ruleset, uint256 usedAmount)
    {
        ...
        uint256 _newUsedSurplusAllowanceOf =
            usedSurplusAllowanceOf[msg.sender][_projectId][_accountingContext.token][ruleset.id][_currency] + _amount;

        // There must be sufficient surplus allowance available.
        uint256 _surplusAllowance = IJBController(address(DIRECTORY.controllerOf(_projectId))).fundAccessLimits()
            .surplusAllowanceOf(_projectId, ruleset.id, msg.sender, _accountingContext.token, _currency);
    ...
    /// @param _amount The amount of terminal tokens added, as a fixed point number with the same amount of decimals as
    /// its relative terminal.
    function recordAddedBalanceFor(uint256 _projectId, address _token, uint256 _amount) external override {
        // Increment the balance.
        balanceOf[msg.sender][_projectId][_token] = balanceOf[msg.sender][_projectId][_token] + _amount;
    }
    ...
    /// @param _tokenCount The number of tokens to make the calculation with, as a fixed point number with 18 decimals.
    /// @param _totalSupply The total supply of tokens to make the calculation with, as a fixed point number with 18
    /// decimals.
        ...
        returns (uint256)
    {
        ...
        return mulDiv(
            ...
            _ruleset.redemptionRate()
                + mulDiv(_tokenCount, JBConstants.MAX_REDEMPTION_RATE - _ruleset.redemptionRate(), _totalSupply),
    ...
    /// @return surplus The surplus of funds in terms of `_targetCurrency`, as a fixed point number with
    /// `_targetDecimals` decimals.
    ...
    /// @param _accountingContext The accounting context of the token whose balance should contribute to the surplus
    /// being measured.
        ...
        returns (uint256 surplus)
    {
        ...
        surplus = (surplus == 0 || _accountingContext.currency == _targetCurrency)
            ...
            : mulDiv(
                ...
                PRICES.pricePerUnitOf(_projectId, _accountingContext.currency, _targetCurrency, _MAX_FIXED_POINT_FIDELITY)
            );

        // Get a reference to the payout limit during the ruleset for the token.
        JBCurrencyAmount[] memory _payoutLimits = IJBController(address(DIRECTORY.controllerOf(_projectId)))
            .fundAccessLimits().payoutLimitsOf(_projectId, _ruleset.id, address(_terminal), _accountingContext.token);
        ...
        for (uint256 _i; _i < _numberOfPayoutLimits;) {
            ...
            _payoutLimit.amount = _payoutLimit.amount
                - usedPayoutLimitOf[_terminal][_projectId][_accountingContext.token][_ruleset.cycleNumber][_payoutLimit
                    .currency];
            ...
            _payoutLimit.amount = _accountingContext.decimals == _targetDecimals
                ? _payoutLimit.amount
                : JBFixedPointNumber.adjustDecimals(_payoutLimit.amount, _accountingContext.decimals, _targetDecimals);

            // Convert the `_payoutLimit`'s amount to be in terms of the provided currency.
            _payoutLimit.amount = _payoutLimit.amount == 0 || _payoutLimit.currency == _targetCurrency
                ? _payoutLimit.amount
                : mulDiv(
                    _payoutLimit.amount,
                    10 ** _MAX_FIXED_POINT_FIDELITY, // Use `_MAX_FIXED_POINT_FIDELITY` to keep as much of the
                        // `_payoutLimitRemaining`'s fidelity as possible when converting.
                    PRICES.pricePerUnitOf(_projectId, _payoutLimit.currency, _targetCurrency, _MAX_FIXED_POINT_FIDELITY)
                );
    ...
    /// @return surplus The total surplus of a project's funds in terms of `_currency`, as a fixed point number with the
