...
contract JBMultiTerminal is JBOperatable, Ownable, IJBMultiTerminal {
    ...
    mapping(uint256 => mapping(address => JBAccountingContext))
    ...
    function heldFeesOf(
        uint256 _projectId
    ) external view override returns (JBFee[] memory) {
        ...
        returns (uint256 balance)
    {
        ...
        if (balance != 0) {
            ...
            _to.addToBalanceOf{value: _payValue}(
                _projectId,
                _token,
                balance,
                false,
        ...
        )
    {
        ...
        uint256 _numberOfAccountingContextsConfigs = _accountingContextConfigs
            .length;
        ...
        for (uint256 _i; _i < _numberOfAccountingContextsConfigs; ) {
            ...
            JBAccountingContext
                storage _accountingContext = _accountingContextForTokenOf[
                    _projectId
            ...
            _accountingContext.currency = uint32(
                uint160(_accountingContextConfig.token)
            );
    ...
    function _acceptedTokenAmountFor(
        ...
        if (
            _accountingContextForTokenOf[_projectId][_token].token == address(0)
    ...
    function _pay(
        ...
        // Scoped section prevents stack too deep. `_delegateAllocations` and `_tokenCount` only used within scope.
        {
            ...
            JBAccountingContext memory _context = _accountingContextForTokenOf[
                _projectId
            ][_token];
            ...
            // Record the payment.
            (_fundingCycle, _tokenCount, _delegateAllocations) = STORE
                .recordPaymentFrom(
                    _payer,
                    _tokenAmount,
                    _projectId,
    ...
    function _redeemTokensOf(
        ...
        // Scoped section prevents stack too deep.
        {
            ...
            (_fundingCycle, reclaimAmount, _delegateAllocations) = STORE
                .recordRedemptionFor(
                    _holder,
                    _projectId,
            ...
            if (_tokenCount != 0)
                IJBController3_1(DIRECTORY.controllerOf(_projectId))
                    .burnTokensOf(_holder, _projectId, _tokenCount, "");
            ...
            if (_delegateAllocations.length != 0) {
                // Get a reference to the token's accounting context.
                JBAccountingContext
                    memory _context = _accountingContextForTokenOf[_projectId][
                        _token
                    ];
    ...
    function _distributeToPayoutSplitsOf(
        ...
        JBSplit[] memory _splits = SPLITS.splitsOf(
            _projectId,
            _domain,
    ...
    function _distributeToPayoutSplit(
        ...
        if (_split.allocator != IJBSplitAllocator(address(0))) {
            ...
            if (
                _feePercent != 0 && !isFeelessAddress[address(_split.allocator)]
            ) {
                unchecked {
            ...
            if (
                ...
                uint256 _payValue = _token == JBTokens.ETH
                    ? netPayoutAmount
                    : 0;
                ...
                {} catch (bytes memory __reason) {
                    _reason = __reason.length == 0
                        ? abi.encode("Allocate fail")
                        : __reason;
                }
            } else {
            ...
            if (_terminal == IJBPaymentTerminal(address(0))) {
                ...
                if (_split.preferAddToBalance) {
                    bytes memory _metadata = bytes(
                        abi.encodePacked(_projectId)
                    );
                    try
    ...
    function _fulfillPayDelegateAllocationsFor(
        ...
        for (uint256 _i; _i < _numberOfAllocations; ) {
            ...
            uint256 _payValue = _tokenAmount.token == JBTokens.ETH
                ? _allocation.amount
                : 0;
    ...
    function _fulfillRedemptionDelegateAllocationsFor(
        ...
        for (uint256 _i; _i < _numberOfAllocations; ) {
            ...
            uint256 _payValue = _beneficiaryTokenAmount.token == JBTokens.ETH
                ? _allocation.amount
                : 0;
    ...
    function _takeFeeFrom(
        ...
        if (_shouldHoldFees) {
            ...
            _heldFeesOf[_projectId].push(
                JBFee(_amount, uint32(_feePercent), _beneficiary)
            );
    ...
    function _processFee(
        ...
        {} catch (bytes memory _reason) {
            _revertTransferFrom(
                _projectId,
                _token,
                address(_feeTerminal) != address(this)
            ...
            if (leftoverAmount == 0)
                ...
                if (leftoverAmount >= _heldFees[_i].amount - _feeAmount) {
                    unchecked {
                        leftoverAmount =
                            leftoverAmount -
                            (_heldFees[_i].amount - _feeAmount);
