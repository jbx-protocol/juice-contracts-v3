...
/// @notice Generic terminal managing all inflows and outflows of funds into the protocol ecosystem. Accomodates rebasing tokens.
  ...
  function _redeemTokensOf(
    ...
    // Scoped section prevents stack too deep. `_feeEligibleDistributionAmount`, `_feeDiscount` and `_feePercent` only used within scope.
    {
      ...
      // Scoped section prevents stack too deep. `_delegateAllocations` only used within scope.
      {
        ...
        // Get a reference to the balance of tokens before recording the redemption.
        uint256 _recordedBalanceBefore = IJBSingleTokenPaymentTerminalStore3_1_1(store).balanceOf(
          this,
          _projectId
        );
        ...
        if (_delegateAllocations.length != 0) {
          ...
          for (uint256 _i; _i < _numDelegates; ) {
            ...
            _data.forwardedAmount.value = (_tokenBalance == _recordedBalanceBefore)
              ? _delegateAllocation.amount
              : PRBMath.mulDiv(_delegateAllocation.amount, _recordedBalanceBefore, _tokenBalance);
