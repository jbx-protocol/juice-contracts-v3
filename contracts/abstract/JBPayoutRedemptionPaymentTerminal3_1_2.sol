    ...
    IJBPayoutRedemptionPaymentTerminal3_1
{
        ...
        returns (uint256)
    {
        ...
        uint256 _overflow =
            IJBSingleTokenPaymentTerminalStore3_1_1(store).currentOverflowOf(this, _projectId);

        // Adjust the decimals of the fixed point number if needed to have 18 decimals.
        uint256 _adjustedOverflow =
            decimals == 18 ? _overflow : JBFixedPointNumber.adjustDecimals(_overflow, decimals, 18);

        // Return the amount converted to ETH.
        return currency == JBCurrencies.ETH
            ? _adjustedOverflow
            : PRBMath.mulDiv(
                _adjustedOverflow,
                10 ** decimals,
        ...
        returns (uint256 reclaimAmount)
    {
        ...
        return _redeemTokensOf(
            _holder, _projectId, _tokenCount, _minReturnedTokens, _beneficiary, _memo, _metadata
        );
    }
        ...
        returns (uint256 netDistributedAmount)
    {
        ...
        return _useAllowanceOf(
            _projectId, _amount, _currency, _minReturnedTokens, _beneficiary, _memo, _metadata
        );
    }
    ...
    function _redeemTokensOf(
        ...
        // Scoped section prevents stack too deep. `_feeEligibleDistributionAmount` and `_feePercent` only used within scope.
        {
            ...
            // Scoped section prevents stack too deep. `_delegateAllocations` only used within scope.
            {
                ...
                (_fundingCycle, reclaimAmount, _delegateAllocations, _memo) =
                IJBSingleTokenPaymentTerminalStore3_1_1(store).recordRedemptionFor(
                    _holder, _projectId, _tokenCount, _memo, _metadata
                );
                ...
                _feePercent = isFeelessAddress[_beneficiary]
                    ...
                    || _feeTerminal == IJBPaymentTerminal(address(0)) ? 0 : fee;
                ...
                if (_tokenCount != 0) {
                    IJBController3_1(directory.controllerOf(_projectId)).burnTokensOf(
                        _holder, _projectId, _tokenCount, "", false
                    );
                }
                ...
                if (_delegateAllocations.length != 0) {
                    ...
                    for (uint256 _i; _i < _numDelegates;) {
                        ...
                        // Trigger any inherited pre-transfer logic.
                        _beforeTransferTo(
                            address(_delegateAllocation.delegate), _delegateAllocation.amount
                        );
            ...
            if (reclaimAmount != 0) {
                ...
                uint256 _reclaimAmountFee =
                    _feePercent == 0 ? 0 : JBFees.feeIn(reclaimAmount, _feePercent);
    ...
    function _distributePayoutsOf(
        ...
        (JBFundingCycle memory _fundingCycle, uint256 _distributedAmount) =
        ...
        // Scoped section prevents stack too deep. `_feeTerminal` only used within scope.
        {
            ...
            // Scoped section prevents stack too deep. `_feeEligibleDistributionAmount` and `_leftoverDistributionAmount` only used within scope.
            {
                ...
                (_leftoverDistributionAmount, _feeEligibleDistributionAmount) =
                ...
                if (_leftoverDistributionAmount != 0) {
                    ...
                    netLeftoverDistributionAmount = _leftoverDistributionAmount
                        - (
                            _feePercent == 0
                                ? 0
                                : JBFees.feeIn(_leftoverDistributionAmount, _feePercent)
                        );
    ...
    function _distributeToPayoutSplitsOf(
        ...
        for (uint256 _i; _i < _splits.length;) {
            ...
            uint256 _netPayoutAmount =
                _distributeToPayoutSplit(_split, _projectId, _group, _payoutAmount, _feePercent);
            ...
            emit DistributeToPayoutSplit(
                _projectId, _domain, _group, _split, _payoutAmount, _netPayoutAmount, msg.sender
            );
    ...
    function _distributeToPayoutSplit(
        ...
        if (_split.allocator != IJBSplitAllocator(address(0))) {
            ...
            JBSplitAllocationData memory _data =
                JBSplitAllocationData(token, netPayoutAmount, decimals, _projectId, _group, _split);
            ...
            if (
                ...
                // If this terminal's token is ETH, send it in msg.value.
                try _split.allocator.allocate{value: token == JBTokens.ETH ? netPayoutAmount : 0}(
                    _data
            ...
            if (_terminal == IJBPaymentTerminal(address(0))) {
                ...
                if (_split.preferAddToBalance) {
                    try _terminal.addToBalanceOf{value: token == JBTokens.ETH ? netPayoutAmount : 0}(
                        _split.projectId,
                        netPayoutAmount,
                        token,
                        "",
                        ...
                        // Revert the payout.
                        _revertTransferFrom(
                            _projectId, address(_terminal), netPayoutAmount, _amount
                        );
                    ...
                    try _terminal.pay{value: token == JBTokens.ETH ? netPayoutAmount : 0}(
                        _split.projectId,
                        netPayoutAmount,
                        token,
    ...
    function _processFee(
        ...
        if (address(_feeTerminal) != address(this)) {
            _beforeTransferTo(address(_feeTerminal), _amount);
        }
        ...
        // Scoped section prevents stack too deep. `_delegateAllocations` and `_tokenCount` only used within scope.
        {
            ...
            if (_tokenCount != 0) {
                ...
                beneficiaryTokenCount = IJBController3_1(directory.controllerOf(_projectId))
                    .mintTokensOf(_projectId, _tokenCount, _beneficiary, "", _preferClaimedTokens, true);
            }
            ...
            if (_delegateAllocations.length != 0) {
                ...
                for (uint256 _i; _i < _numDelegates;) {
                    ...
                    emit DelegateDidPay(
                        _delegateAllocation.delegate, _data, _delegateAllocation.amount, msg.sender
                    );
    ...
    function _addToBalanceOf(
        ...
        IJBSingleTokenPaymentTerminalStore3_1_1(store).recordAddedBalanceFor(
            _projectId, _amount + _refundedFees
        );
