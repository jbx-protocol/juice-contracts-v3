    ...
    IJBSingleTokenPaymentTerminalStore3_1_1
{
    ...
    function _overflowDuring(
        ...
        for (uint256 _i; _i < _numberOfDistributionLimits;) {
            ...
            uint256 _distributionLimitRemaining = _distributionLimit.value
                - usedDistributionLimitOf[_terminal][_projectId][_fundingCycle.number][_distributionLimit
                    .currency];

            // Convert the _distributionRemaining to be in terms of the provided currency.
            _normalizedDistributionLimitsRemaining += (
                _distributionLimitRemaining == 0 || _distributionLimit.currency == _balanceCurrency
            )
        ...
        unchecked {
            return _balanceOf > _normalizedDistributionLimitsRemaining
        ...
        returns (uint256)
    {
        ...
        uint256 _totalOverflow18Decimal = _currency == JBCurrencies.ETH
            ? _ethOverflow
            : PRBMath.mulDiv(
                _ethOverflow, 10 ** 18, prices.priceFor(_projectId, JBCurrencies.ETH, _currency, 18)
            );
