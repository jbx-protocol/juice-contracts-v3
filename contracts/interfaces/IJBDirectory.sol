...
interface IJBDirectory {
    ...
    event SetIsAllowedToSetFirstController(
    ...
    function isAllowedToSetFirstController(
        address account
    ) external view returns (bool);

    function terminalsOf(
        uint256 projectId
    ) external view returns (IJBPaymentTerminal[] memory);
    ...
    function setTerminalsOf(
        uint256 projectId,
        IJBPaymentTerminal[] calldata terminals
    ) external;
