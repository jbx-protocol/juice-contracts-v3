...
contract JBController is JBPermissioned, ERC2771Context, ERC165, IJBController, IJBMigratable {
    ...
    /// project.
    /// @custom:param _projectId The ID of the project to get the pending reserved token balance of.
    mapping(uint256 _projectId => uint256) public override pendingReservedTokenBalanceOf;
    ...
    mapping(uint256 _projectId => string) public override metadataOf;
    ...
    /// to splits yet.
    function totalTokenSupplyWithReservedTokensOf(uint256 _projectId) external view override returns (uint256) {
    ...
    function supportsInterface(bytes4 _interfaceId) public view virtual override(ERC165, IERC165) returns (bool) {
        return _interfaceId == type(IJBController).interfaceId
            ...
            || _interfaceId == type(IJBMigratable).interfaceId || _interfaceId == type(IJBPermissioned).interfaceId
    ...
    /// @param _projectMetadata Metadata to associate with the project. This can be updated any time by the owner of the
    /// project.
        ...
        returns (uint256 beneficiaryTokenCount)
    {
        ...
        // Scoped section prevents stack too deep. `_ruleset` only used within scope.
        {
            ...
            if (
                !_ruleset.allowOwnerMinting() && !directory.isTerminalOf(_projectId, IJBTerminal(_msgSender()))
                    && _msgSender() != address(_ruleset.dataHook())
        ...
        if (_reservedRate != JBConstants.MAX_RESERVED_RATE) {
            ...
            beneficiaryTokenCount =
                mulDiv(_tokenCount, JBConstants.MAX_RESERVED_RATE - _reservedRate, JBConstants.MAX_RESERVED_RATE);
        ...
        emit MintTokens(
            _beneficiary, _projectId, _tokenCount, beneficiaryTokenCount, _memo, _reservedRate, _msgSender()
        );
    }
    ...
    function receiveMigrationFrom(IERC165 _from, uint256 _projectId) external virtual override {
        ...
        if (
            ...
            metadataOf[_projectId] = IJBProjectMetadataRegistry(address(_from)).metadataOf(_projectId);
        }
    }
    ...
    function setMetadataOf(
        ...
        external
        override
        requirePermission(projects.ownerOf(_projectId), _projectId, JBPermissionIds.SET_PROJECT_METADATA)
    {
        ...
        returns (uint256 tokenCount)
    {
        ...
        uint256 _leftoverTokenCount = tokenCount == 0
            ? 0
            : _sendTokensToSplitGroupOf(_projectId, _ruleset.id, JBSplitGroupIds.RESERVED_TOKENS, tokenCount);
        ...
        emit SendReservedTokensToSplits(
            _ruleset.id, _ruleset.cycleNumber, _projectId, _owner, tokenCount, _leftoverTokenCount, _memo, _msgSender()
        );
    }
        ...
        returns (uint256 leftoverAmount)
    {
        ...
        for (uint256 _i; _i < _numberOfSplits; ++_i) {
            ...
            uint256 _tokenCount = mulDiv(_amount, _split.percent, JBConstants.SPLITS_TOTAL_PERCENT);
            ...
            if (_tokenCount > 0) {
                ...
                if (_split.hook != IJBSplitHook(address(0))) {
                    ...
                    // Process.
                    _split.hook.process(JBSplitHookPayload(_token, _tokenCount, 18, _projectId, _groupId, _split));
                }
        ...
        returns (uint256 rulesetId)
    {
        ...
        for (uint256 _i; _i < _numberOfConfigurations; ++_i) {
            ...
            fundAccessLimits.setFundAccessLimitsFor(_projectId, _ruleset.id, _rulesetConfig.fundAccessLimitGroups);
    ...
    function _configureTerminals(uint256 _projectId, JBTerminalConfig[] calldata _terminalConfigs) internal {
        ...
        for (uint256 _i; _i < _numberOfTerminalConfigs; ++_i) {
            ...
            _terminalConfig.terminal.addAccountingContextsFor(_projectId, _terminalConfig.accountingContextConfigs);
