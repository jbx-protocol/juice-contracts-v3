...
import jbFundingCycleDataSource from '../../artifacts/contracts/interfaces/IJBFundingCycleDataSource3_1_1.sol/IJBFundingCycleDataSource3_1_1.json';
...
describe('JBSingleTokenPaymentTerminalStore3_1_1::recordRedemptionFor(...)', function () {
  const PROJECT_ID = 2;
  ...
  const METADATA1 = ethers.utils.randomBytes(32);
  const METADATA2 = ethers.utils.randomBytes(32);
  ...
  it('Should record redemption with a datasource and emit event', async function () {
    ...
    await mockJbFundingCycleDataSource.mock.redeemParams
      ...
      .returns(AMOUNT, newMemo, [{ delegate: delegate.address, amount: delegateAmount, metadata: METADATA2 }]);
    ...
    expect(returnedValue.delegateAllocations).to.eql([
      [delegate.address, ethers.BigNumber.from(delegateAmount), ethers.utils.hexlify(METADATA2)],
    ]);
  ...
  it(`Can't record redemption with claim amount > total supply`, async function () {
    ...
    await mockJbFundingCycleDataSource.mock.redeemParams
      ...
      .returns(AMOUNT, newMemo, [{ delegate: delegate.address, amount: 0, metadata: METADATA2 }]);
