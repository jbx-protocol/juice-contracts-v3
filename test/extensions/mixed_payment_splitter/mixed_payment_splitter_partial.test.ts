...
describe('MixedPaymentSplitter partial-subscription tests', () => {
    const jbxJbTokensEth = '0x000000000000000000000000000000000000EEEe';

    const payees: string[] = [];
    const projects = [2, 3, 4];
    const shares = [200_000, 100_000, 200_000, 200_000, 100_000];
    ...
    before(async () => {
        ...
        await directory.mock.isTerminalOf.withArgs(5, terminalFour.address).returns(true);
        ...
        await directory.mock.primaryTerminalOf.withArgs(5, jbxJbTokensEth).returns(terminalFour.address);
    ...
    it('Get pending token amount', async () => {
        ...
        expect(await mixedPaymentSplitter['pending(address,uint256)'](token.address, projects[1])).to.equal(largeShare.toString());
        expect(await mixedPaymentSplitter['pending(address,uint256)'](token.address, projects[2])).to.equal(smallShare.toString());
    });

    it('Distribute Ether payment', async () => {
        const largeShare = ethers.utils.parseEther('1.0').mul(200_000).div(1_000_000);
        const initialSplitterBalance = await ethers.provider.getBalance(mixedPaymentSplitter.address);
        ...
        expect(await ethers.provider.getBalance(mixedPaymentSplitter.address)).to.equal(initialSplitterBalance.sub(largeShare.mul(2)));
    });

    it('Distribute token payment', async () => {
        const largeShare = BigNumber.from(1_000_000_000).mul(200_000).div(1_000_000);

        const initialSplitterBalance = await token.balanceOf(mixedPaymentSplitter.address);
        ...
        await mixedPaymentSplitter['pending(address,address)'](token.address, accounts[0].address)
        // await expect(mixedPaymentSplitter['distribute(address,uint256)'](token.address, projects[0])) // TODO
        ...
        expect(await token.balanceOf(mixedPaymentSplitter.address)).to.equal(initialSplitterBalance.sub(largeShare.mul(1)));
    });
    ...
    it('Fail to distribute token payment, nothing due', async () => { // TODO: needs actual terminal
        expect(await mixedPaymentSplitter['pending(address,address)'](token.address, accounts[0].address)).to.equal(0);
    ...
    it('Add payees', async () => {
        await expect(mixedPaymentSplitter.connect(deployer)['addPayee(address,uint256)'](accounts[2].address, 20_000)).not.to.be.reverted;
        await expect(mixedPaymentSplitter.connect(deployer)['addPayee(address,uint256)'](accounts[1].address, 30_000)).not.to.be.reverted;
        await expect(mixedPaymentSplitter.connect(deployer)['addPayee(address,uint256)'](accounts[3].address, 200_000)).to.be.reverted;
        await expect(mixedPaymentSplitter.connect(deployer)['addPayee(uint256,uint256)'](5, 200_000)).to.be.reverted;
        await expect(mixedPaymentSplitter.connect(deployer)['addPayee(uint256,uint256)'](5, 50_000)).not.to.be.reverted;
    });
});
