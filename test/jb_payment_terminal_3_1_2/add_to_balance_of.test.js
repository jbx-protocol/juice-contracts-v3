...
describe('JBPayoutRedemptionPaymentTerminal3_1_2::addToBalanceOf(...)', function () {
  ...
  const PROJECT_ID = 2;
  const AMOUNT = ethers.utils.parseEther('1025');
  ...
  it('Should add to the project balance, refund any held fee if _shouldRefundHeldFees is true, and remove them if the transferred amount is enough, and emit event', async function () {
    const {
      caller,
      beneficiaryOne,
      beneficiaryTwo,
    ...
    let feeNetAmount = ethers.BigNumber.from(heldFee[0].amount).sub(
      ethers.BigNumber.from(heldFee[0].amount).mul(MAX_FEE).div(ethers.BigNumber.from(heldFee[0].fee).add(MAX_FEE)),
    );
    await mockJBPaymentTerminalStore.mock.recordAddedBalanceFor
  ...
  it('Should add to the project balance and not refund held fee if _shouldRefundHeldFees is false, and emit event', async function () {
    const {
      caller,
      terminalOwner,
      beneficiaryOne,
  ...
  it('Should work with non-eth terminal if no value is sent', async function () {
    ...
    await JBERC20PaymentTerminal.connect(caller)[
      'addToBalanceOf(uint256,uint256,address,string,bytes)'
  ...
  it('Should add to the project balance, partially refund a held fee and substract the amount from the held fee amount and emit event', async function () {
    ...
    let paidAmount = ethers.BigNumber.from('10500000');
    let feeNetAmount = paidAmount.mul(ethers.BigNumber.from(heldFee[0].fee)).div(MAX_FEE);
    ...
    await expect(
      ...
      .withArgs(PROJECT_ID, paidAmount /*amount*/, feeNetAmount /*refund*/, 0 /*leftOver*/, caller.address);

    let heldFeeAfter = await jbEthPaymentTerminal.heldFeesOf(PROJECT_ID);

    expect(heldFeeAfter[0].amount).to.equal(heldFeeBefore[0].amount.sub(paidAmount).sub(feeNetAmount));
  });
  ...
  it('Should add to the project balance, refund multiple held fee by substracting the amount from the held fee amount when possible and emit event', async function () {
    ...
    let feeNetAmount = ethers.BigNumber.from(AMOUNT).sub(
      ethers.BigNumber.from(AMOUNT).mul(MAX_FEE).div(ethers.BigNumber.from(heldFee[0].fee).add(MAX_FEE)),
    );

    let amountAfterFees = AMOUNT.sub(feeNetAmount).mul(2);
    let diff = ethers.BigNumber.from('1000');
    let paidAmount = amountAfterFees.sub(diff);

    await mockJBPaymentTerminalStore.mock.recordAddedBalanceFor
      .withArgs(PROJECT_ID, paidAmount.add(paidAmount.mul(ethers.BigNumber.from(heldFee[0].fee)).div(MAX_FEE)))
      .returns();
    ...
    expect(
      ...
      .withArgs(
        PROJECT_ID,
        paidAmount,
        paidAmount.mul(ethers.BigNumber.from(heldFee[0].fee)).div(MAX_FEE),
        MEMO,
        METADATA,
        caller.address,
      )
      ...
      .withArgs(
        PROJECT_ID,
        paidAmount /*amount*/,
        paidAmount.mul(ethers.BigNumber.from(heldFee[0].fee)).div(MAX_FEE) /*refund*/,
        0 /*leftOver*/,
        caller.address,
      );
    ...
    expect(heldFeeAfter[0].amount).to.equal(diff.add(diff.mul(ethers.BigNumber.from(heldFee[0].fee)).div(MAX_FEE)));
  });
  ...
  it('Should add to the project balance, refund one out of multiple held fees bigger than the amount, keep the held fee difference and emit event', async function () {
    ...
    let feeNetAmount = ethers.BigNumber.from(amountToAdd).mul(ethers.BigNumber.from(heldFee[0].fee)).div(MAX_FEE);
    ...
    expect(heldFeeAfter.length).to.equal(2);
    expect(heldFeeAfter[0].amount).to.equal(AMOUNT.sub(amountToAdd.add(feeNetAmount)));
  });
  ...
  it('Should add to the project balance, refund all the held fees if the amount to add to balance if bigger and emit event', async function () {
    ...
    let netHeldFee = ethers.BigNumber.from(heldFeeBefore[0].amount).sub(
      ethers.BigNumber.from(heldFeeBefore[0].amount).mul(MAX_FEE).div(ethers.BigNumber.from(heldFeeBefore[0].fee).add(MAX_FEE)),
    );
  ...
  it("Can't add with value if terminal token isn't ETH", async function () {
    ...
    await expect(
      JBERC20PaymentTerminal.connect(caller)[
        'addToBalanceOf(uint256,uint256,address,string,bytes)'
      ](PROJECT_ID, AMOUNT, mockToken.address, MEMO, METADATA, {
        value: 10,
      }),
