...
describe('JBPayoutRedemptionPaymentTerminal3_1_2::distributePayoutsOf(...)', function () {
      ...
      splits.map(async (split) => {
        await expect(tx)
          ...
          .withArgs(
            ...
            /*netPayout*/ Math.floor((AMOUNT_DISTRIBUTED * split.percent) / SPLITS_TOTAL_PERCENT),
            caller.address,
          );
      }),
    );
  ...
  it('Should distribute payout and use the allocator if set in splits', async function () {
    ...
    await Promise.all(
      splits.map(async (split) => {
        // NetAmount and Amount are the same since the fee was set to 0 
        await expect(tx)
          .to.emit(jbEthPaymentTerminal, 'DistributeToPayoutSplit')
          .withArgs(
            PROJECT_ID,
            ...
            /*NetAmount*/ Math.floor((AMOUNT_DISTRIBUTED * split.percent) / SPLITS_TOTAL_PERCENT),
            caller.address,
          );
      }),
    );
  ...
  it('Should distribute payout using the terminal of the project if project id is set in splits', async function () {
    const {
      projectOwner,
      terminalOwner,
      caller,
  ...
  it('Should not have any leftover (fix rounding error)', async function () {
    const {
      projectOwner,
      terminalOwner,
      caller,
    ...
    await Promise.all(
      splits.map(async (split, i) => {
        // If this is the last split it receives the remainder incl rounding errors
        var amountToReceive;
        if (i == splits.length - 1) {
          // We calc the amount send to the other splits, subtracting that from the total amount will give the amount this split will receive
          amountToReceive =
            AMOUNT_TO_DISTRIBUTE -
            Math.floor(
              (AMOUNT_TO_DISTRIBUTE * (1000000000 - split.percent)) / SPLITS_TOTAL_PERCENT,
            );
        } else {
          amountToReceive = Math.floor(
            (AMOUNT_TO_DISTRIBUTE * split.percent) / SPLITS_TOTAL_PERCENT,
          );
        }
      ...
      splits.map(async (split) => {
        await expect(tx)
          ...
          .withArgs(
            ...
            /*Amount*/ Math.floor((0 * split.percent) / SPLITS_TOTAL_PERCENT),
            /*NetAmount*/ Math.floor((0 * split.percent) / SPLITS_TOTAL_PERCENT),
            caller.address,
          );
      }),
    );
