...
describe.only("JBRulesets::configureFor(...)", function () {
  const PROJECT_ID = 2;

  const EMPTY_FUNDING_CYCLE = {
    number: ethers.BigNumber.from(0),
    rulesetId: ethers.BigNumber.from(0),
  ...
  const MAX_DECAY_RATE = ethers.BigNumber.from(1000000000);
  ...
  async function setup() {
    ...
    const mockBallot = await deployMockContract(
      deployer,
      ijbFundingCycleBallot.abi
    );
    await mockBallot.mock.supportsInterface.returns(true);

    const jbFundingCycleStoreFactory = await ethers.getContractFactory(
      "contracts/JBRulesets.sol:JBRulesets"
    );
    const jbFundingCycleStore = await jbFundingCycleStoreFactory.deploy(
  ...
  it("Should have no current or queued funding cycle before configuring", async function () {
    ...
    expect(await jbFundingCycleStore.currentBallotStateOf(PROJECT_ID)).to.eql(
      ballotStatus.EMPTY
    );

    const [latestFundingCycle, ballotState] =
      await jbFundingCycleStore.latestConfiguredOf(PROJECT_ID);
    expect(cleanFundingCycle(latestFundingCycle)).to.eql(EMPTY_FUNDING_CYCLE);
    expect(ballotState).to.deep.eql(ballotStatus.EMPTY);
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql(EMPTY_FUNDING_CYCLE);
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql(EMPTY_FUNDING_CYCLE);
  });
  ...
  it("Should create current funding cycle and queued cycle on first configure", async function () {
    ...
    const configurationTimestamp = await getTimestamp(
      configureForTx.blockNumber
    );
    ...
    await expect(configureForTx)
      ...
      .withArgs(
        ...
        [
          ...
          DEFAULT_FUNDING_CYCLE_DATA.decayRate,
    ...
    expect(
      cleanFundingCycle(
        await jbFundingCycleStore.get(PROJECT_ID, configurationTimestamp)
      )
    ).to.eql(expectedCurrentFundingCycle);

    let [latestFundingCycle, ballotState] =
      await jbFundingCycleStore.latestConfiguredOf(PROJECT_ID);
    ...
    expect(
      ...
      ...expectedCurrentFundingCycle,
      number: expectedCurrentFundingCycle.cycleNumber.add(1), // next number
      start: expectedCurrentFundingCycle.start.add(
        expectedCurrentFundingCycle.duration
      ), // starts at the end of the first cycle
    });
    ...
    await fastForward(
      configureForTx.blockNumber,
      DEFAULT_FUNDING_CYCLE_DATA.duration.sub(2)
    );

    [latestFundingCycle, ballotState] =
      await jbFundingCycleStore.latestConfiguredOf(PROJECT_ID);
    expect(cleanFundingCycle(latestFundingCycle)).to.eql(
      expectedCurrentFundingCycle
    );
    ...
    expect(
      ...
      ...expectedCurrentFundingCycle,
      number: expectedCurrentFundingCycle.cycleNumber.add(2), // next number
    ...
    await fastForward(
      configureForTx.blockNumber,
      DEFAULT_FUNDING_CYCLE_DATA.duration.mul(2)
    );
    ...
    expect(
      ...
      ...expectedCurrentFundingCycle,
      number: expectedCurrentFundingCycle.cycleNumber.add(3), // next number
      start: expectedCurrentFundingCycle.start
        .add(expectedCurrentFundingCycle.duration)
        .add(expectedCurrentFundingCycle.duration)
        .add(expectedCurrentFundingCycle.duration), // starts at the end of the second cycle
  ...
  it("Should configure subsequent cycle during a funding cycle", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore, mockBallot } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
    ...
    const secondFundingCycleData = createFundingCycleData({
      ...
      decayRate: firstFundingCycleData.decayRate.add(1),
    ...
    await fastForward(
      firstConfigureForTx.blockNumber,
      firstFundingCycleData.duration.sub(5)
    );
    ...
    const secondConfigurationTimestamp = await getTimestamp(
      secondConfigureForTx.blockNumber
    );
    ...
    const expectedSecondFundingCycle = {
      ...
      rulesetId: secondConfigurationTimestamp,
      ...
      decayRate: secondFundingCycleData.decayRate,
    ...
    expect(
      cleanFundingCycle(
        await jbFundingCycleStore.get(PROJECT_ID, secondConfigurationTimestamp)
      )
    ...
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql(expectedFirstFundingCycle);
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql(expectedSecondFundingCycle);
  });
  ...
  it("Should configure subsequent cycle that starts in the future", async function () {
    ...
    const reconfiguredFundingCycleMustStartOnOrAfter =
    ...
    const expectedSecondFundingCycle = {
      ...
      rulesetId: secondConfigurationTimestamp,
      basedOn: firstConfigurationTimestamp, // based on the first cycle
      start: firstConfigurationTimestamp.add(
        firstFundingCycleData.duration.mul(2)
      ), // starts at the end of the second cycle
      duration: secondFundingCycleData.duration,
      weight: secondFundingCycleData.weight,
      decayRate: secondFundingCycleData.decayRate,
      ballot: secondFundingCycleData.ballot,
    ...
    expect(
      ...
      number: expectedFirstFundingCycle.cycleNumber.add(1),
  ...
  it("Should configure subsequent short cycle that starts in the future after another iteration of the current", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore } = await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
  ...
  it("Should configure multiple subsequent cycles that start in the future", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore } = await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
    ...
    const thirdFundingCycleData = createFundingCycleData({
      ...
      decayRate: firstFundingCycleData.decayRate.add(2),
      weight: firstFundingCycleData.weight.add(2),
    });

    const secondFundingCycleMustStartOnOrAfter =
      firstConfigurationTimestamp.add(firstFundingCycleData.duration);

    const thirdFundingCycleMustStartOnOrAfter = firstConfigurationTimestamp
    ...
    const thirdConfigurationTimestamp = await getTimestamp(
      thirdConfigureForTx.blockNumber
    );
    ...
    await expect(thirdConfigureForTx)
      ...
      .withArgs(
        ...
        PROJECT_ID,
        /*basedOn=*/ secondConfigurationTimestamp
      );

    const expectedThirdFundingCycle = {
      number: ethers.BigNumber.from(3), // third cycle
      rulesetId: thirdConfigurationTimestamp,
      basedOn: secondConfigurationTimestamp, // based on the second cycle
      start: secondConfigurationTimestamp.add(secondFundingCycleData.duration), // starts at the end of the second cycle
      duration: thirdFundingCycleData.duration,
      weight: thirdFundingCycleData.weight,
      decayRate: thirdFundingCycleData.decayRate,
      ballot: thirdFundingCycleData.ballot,
      metadata: RANDOM_FUNDING_CYCLE_METADATA_2,
    };

    // Current shows the first still.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql(expectedFirstFundingCycle);

    // Queued shows the second still.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql({
      ...expectedSecondFundingCycle,
    ...
    // Current shows the second still.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql({
      ...expectedSecondFundingCycle,
    ...
    expect(
      ...
      number: expectedFirstFundingCycle.cycleNumber.add(2),
      start: expectedFirstFundingCycle.start
        .add(expectedFirstFundingCycle.duration)
        .add(expectedSecondFundingCycle.duration),
    });
  });
  ...
  it("Should configure multiple subsequent cycles that start in the future with standby ballots that turn failed", async function () {
    ...
    const expectedThirdFundingCycle = {
      ...
      start: expectedFirstFundingCycle.start
        .add(expectedFirstFundingCycle.duration)
        .add(expectedSecondFundingCycle.duration), // starts at the end of the second cycle
      duration: thirdFundingCycleData.duration,
      weight: thirdFundingCycleData.weight,
      decayRate: thirdFundingCycleData.decayRate,
      ballot: thirdFundingCycleData.ballot,
    ...
    await mockBallot.mock.stateOf
      .withArgs(
        PROJECT_ID,
        thirdConfigurationTimestamp,
        firstConfigurationTimestamp.add(firstFundingCycleData.duration.mul(3))
      )
    ...
    expect(
      ...
      number: expectedFirstFundingCycle.cycleNumber.add(2),
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration.mul(2)
      ),
    });

    // fast forward another cycle
    await fastForward(
      firstConfigureForTx.blockNumber,
      firstFundingCycleData.duration.mul(2)
    );

    // Current shows the first rolled over.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql({
      ...expectedFirstFundingCycle,
    ...
    // Queued shows the first rolled over again.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql({
      ...expectedFirstFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(3),
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration.mul(3)
      ),
    });
  });

  it("Should configure multiple subsequent cycles that start in the future with standby ballots and approved other ballots", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore, mockBallot } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
    ...
    await fastForward(
      firstConfigureForTx.blockNumber,
      firstFundingCycleData.duration.mul(3)
    );
    ...
    expect(
      ...
      ...expectedThirdFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(4),
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration
          .mul(3)
          .add(expectedThirdFundingCycle.duration)
      ),
    });
  });
  ...
  it("Should configure multiple subsequent cycles that start in the future with standby ballots that turn failed with deeper nesting", async function () {
    ...
    const fourthFundingCycleData = createFundingCycleData({
      duration: firstFundingCycleData.duration.add(3),
      weight: firstFundingCycleData.weight.add(3),
    });
    ...
    const fourthFundingCycleMustStartOnOrAfter = firstConfigurationTimestamp
      .add(firstFundingCycleData.duration)
      .add(firstFundingCycleData.duration)
      .add(firstFundingCycleData.duration)
      .add(thirdFundingCycleData.duration);
    ...
    // Configure fourth funding cycle
    const fourthConfigureForTx = await jbFundingCycleStore
      .connect(controller)
      .configureFor(
        PROJECT_ID,
        fourthFundingCycleData,
        RANDOM_FUNDING_CYCLE_METADATA_2,
        fourthFundingCycleMustStartOnOrAfter
      );

    // The timestamp the second configuration was made during.
    const fourthConfigurationTimestamp = await getTimestamp(
      fourthConfigureForTx.blockNumber
    );

    await expect(fourthConfigureForTx)
      .to.emit(jbFundingCycleStore, `Init`)
      .withArgs(
        fourthConfigurationTimestamp,
        PROJECT_ID,
        /*basedOn=*/ thirdConfigurationTimestamp
      );
    ...
    // fast forward yet another cycle
    await fastForward(
      firstConfigureForTx.blockNumber,
      firstFundingCycleData.duration.mul(3)
    );

    const expectedFourthFundingCycle = {
      rulesetId: fourthConfigurationTimestamp,
      basedOn: thirdConfigurationTimestamp, // based on the third cycle
      start: firstConfigurationTimestamp
        .add(firstFundingCycleData.duration.mul(3))
        .add(thirdFundingCycleData.duration), // starts at the end of the second cycle
      duration: fourthFundingCycleData.duration,
      weight: fourthFundingCycleData.weight,
      decayRate: fourthFundingCycleData.decayRate,
      ballot: fourthFundingCycleData.ballot,
      metadata: RANDOM_FUNDING_CYCLE_METADATA_2,
    };

    // Current shows the first rolled over twice.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql({
      ...expectedFirstFundingCycle,
    ...
    expect(
      ...
      ...expectedFourthFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(5),
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration
          .mul(3)
          .add(expectedThirdFundingCycle.duration)
          .add(expectedFourthFundingCycle.duration)
      ),
    });
  });
  ...
  it("Should configure multiple subsequent cycles that start in the future, while overriding if timestamps dont match up", async function () {
    ...
    // Queued shows the second over fc.
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql({
      ...expectedSecondFundingCycle,
    ...
    const expectedThirdFundingCycle = {
      number: ethers.BigNumber.from(3), // third cycle
      rulesetId: thirdConfigurationTimestamp,
      basedOn: firstConfigurationTimestamp, // based on the second cycle
      start: firstConfigurationTimestamp.add(
        firstFundingCycleData.duration.mul(2)
      ), // starts at the end of the third cycle
      duration: thirdFundingCycleData.duration,
      weight: thirdFundingCycleData.weight,
      decayRate: thirdFundingCycleData.decayRate,
      ballot: thirdFundingCycleData.ballot,
  ...
  it("Should configure subsequent cycle that starts in the future if current cycle has no duration", async function () {
    ...
    const reconfiguredFundingCycleMustStartOnOrAfter =
      firstConfigurationTimestamp.add(reconfiguredFundingCycleStartsIn);
  ...
  it("Should configure subsequent cycle during a rolled over funding cycle", async function () {
    ...
    expect(
      ...
      ...expectedFirstFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(cycleDiff.sub(1)),
      start: expectedFirstFundingCycle.start.add(
  ...
  it("Should configure subsequent cycle during a rolled over funding cycle with approved ballot", async function () {
    ...
    await mockBallot.mock.stateOf
      .withArgs(
        PROJECT_ID,
        secondConfigurationTimestamp,
        firstConfigurationTimestamp.add(
          firstFundingCycleData.duration.mul(cycleDiff)
        )
      )
      .returns(ballotStatus.APPROVED);
  ...
  it("Should not overwrite when configure a subsequent cycle during a funding cycle with approved ballot", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore, mockBallot } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
    ...
    const thirdFundingCycleData = createFundingCycleData({
      ...
      decayRate: secondFundingCycleData.decayRate.add(1),
    ...
    const expectedThirdFundingCycle = {
      number: ethers.BigNumber.from(3), // second cycle
      rulesetId: thirdConfigurationTimestamp,
      basedOn: secondConfigurationTimestamp, // based on the first cycle
      start: firstConfigurationTimestamp
        .add(firstFundingCycleData.duration)
        .add(secondFundingCycleData.duration), // starts at the end of the first cycle
      duration: thirdFundingCycleData.duration,
      weight: thirdFundingCycleData.weight,
      decayRate: thirdFundingCycleData.decayRate,
      ballot: thirdFundingCycleData.ballot,
  ...
  it("Should configure subsequent cycle during a rolled over funding cycle many multiples of duration later", async function () {
    ...
    const decayRate = 0.005; // Use a decay rate of 0.5%

    const firstFundingCycleData = createFundingCycleData({
      duration: ethers.BigNumber.from(5),
      decayRate: MAX_DECAY_RATE.div(1 / decayRate),
    });
    ...
    try {
      expect(
        ...
        weight: firstFundingCycleData.weight.div(1 / decayRate ** cycleDiff),
      });
    } catch {
      expect(
        cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
  ...
  it("Should ignore failed configuration and roll over current configuration", async function () {
    const { controller, mockJbDirectory, mockBallot, jbFundingCycleStore } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
    ...
    // The timestamp the second configuration was made during.
    const failedConfigurationTimestamp = await getTimestamp(
      failedConfigureForTx.blockNumber
    );

    const expectedFailedFundingCycle = {
      number: ethers.BigNumber.from(2),
      rulesetId: failedConfigurationTimestamp,
      basedOn: firstConfigurationTimestamp,
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration
      ),
      ...
      decayRate: failedFundingCycleData.decayRate,
    ...
    expect(
      ...
      ...expectedFirstFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(cycleDiff),
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration.mul(cycleDiff)
      ),
    });
  });
  ...
  it("Should ignore failed configuration and roll over current configuration, and maintains even if ballot later becomes approved", async function () {
    ...
    const decayRate = 0.5; // 50% decay rate
    ...
    expect(
      cleanFundingCycle(await jbFundingCycleStore.currentOf(PROJECT_ID))
    ).to.eql(expectedFailedFundingCycle);
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql({
      ...expectedFailedFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(2),
      start: expectedFailedFundingCycle.start.add(
        expectedFailedFundingCycle.duration
      ),
      weight: expectedFailedFundingCycle.weight.div(1 / decayRate),
    });
  });
  ...
  it("Should configure subsequent cycle during a rolled over funding cycle that overwrote a failed one", async function () {
    ...
    await mockBallot.mock.stateOf
      .withArgs(
        ...
        secondConfigurationTimestamp,
        firstConfigurationTimestamp.add(
          firstFundingCycleData.duration.mul(cycleDiff.add(1))
        )
      )
      .returns(ballotStatus.APPROVED);
  ...
  it("Should configure subsequent cycle during a funding cycle with duration of 0 irrespectively of a decay rate", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore } = await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
  ...
  it("Should not use a funding cycle that fails a ballot", async function () {
    ...
    // Fast forward to the next cycle.
    await fastForward(
      firstConfigurationTimestamp.blockNumber,
      firstFundingCycleData.duration
    );
    ...
    expect(
      ...
      ...expectedFirstFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(1), // next number
      start: expectedFirstFundingCycle.start.add(
        expectedFirstFundingCycle.duration
      ), // starts at the end of the first cycle
    });
    ...
    expect(
      ...
      ...expectedFirstFundingCycle,
      number: expectedFirstFundingCycle.cycleNumber.add(2), // next number
      start: expectedFirstFundingCycle.start
        .add(expectedFirstFundingCycle.duration)
  ...
  it("Should hold off on using a reconfigured funding cycle if the current cycle's ballot duration doesn't end until after the current cycle is over", async function () {
    ...
    expect(
      cleanFundingCycle(await jbFundingCycleStore.queuedOf(PROJECT_ID))
    ).to.eql(expectedReconfiguredFundingCycle);
  });
  ...
  it("Should configure subsequent cycle with a weight derived from previous cycle if a value of 0 is passed in", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore, addrs } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
  ...
  it("Should apply a decay rate to subsequent cycle after reconfiguration with a weight derived from previous cycle if a value of 0 is passed in", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore, addrs } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
    ...
    const expectedSecondFundingCycle = {
      ...
      start: firstConfigurationTimestamp.add(firstFundingCycleData.duration), // starts at the end of the first cycle
      duration: secondFundingCycleData.duration,
      weight: firstFundingCycleData.weight.div(1 / decayRate), // expect a weight derived from the previous cycle's values because 0 was sent.
      decayRate: secondFundingCycleData.decayRate,
      ballot: secondFundingCycleData.ballot,
      metadata: RANDOM_FUNDING_CYCLE_METADATA_2,
    };
  ...
  it("Should apply a decay rate to a subsequent cycle that rolls over", async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore, addrs } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
    ...
    const decayRate = 0.5; // Use a decay rate of 50%
    ...
    const expectedCurrentFundingCycle = {
      ...
      decayRate: fundingCycleData.decayRate,
    ...
    expect(
      ...
      weight: expectedCurrentFundingCycle.weight.div(1 / decayRate), // apply the decay rate
    });
  });
  ...
  it("Should configure subsequent cycle during a rolled over funding cycle overriding an already-proposed configuration", async function () {
    ...
    expect(
      cleanFundingCycle(
        await jbFundingCycleStore.get(PROJECT_ID, thirdConfigurationTimestamp)
      )
    ).to.eql(expectedThirdFundingCycle);
  ...
  it("Should configure subsequent cycle during a rolled over funding cycle without overriding an already-proposed configuration if it has been approved by the ballot", async function () {
    ...
    const expectedThirdFundingCycle = {
      ...
      basedOn: secondConfigurationTimestamp, // based on the first cycle
      start: firstConfigurationTimestamp
        .add(firstFundingCycleData.duration.mul(2))
        .add(secondFundingCycleData.duration), // starts at the end of the 3rd cycle
      duration: thirdFundingCycleData.duration,
      weight: thirdFundingCycleData.weight,
      decayRate: thirdFundingCycleData.decayRate,
      ballot: thirdFundingCycleData.ballot,
  ...
  it(`Can't configure if funding cycle duration is bigger than 2^64 - 1`, async function () {
    ...
    const fundingCycleData = createFundingCycleData({
      duration: ethers.BigNumber.from(2).pow(64),
    });
    ...
    ).to.be.revertedWith(errors.INVALID_RULESET_DURATION);
  });

  it(`Can't configure if funding cycle decay rate is above 100%`, async function () {
    const { controller, mockJbDirectory, jbFundingCycleStore } = await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);
    ...
    const fundingCycleData = createFundingCycleData({
      decayRate: BigNumber.from(1000000001),
    });
    ...
    ).to.be.revertedWith(errors.INVALID_DECAY_RATE);
  });
  ...
  it(`Can't configure if ballot is not a contract`, async function () {
    ...
    ).to.be.revertedWith(errors.INVALID_RULESET_APPROVAL_HOOK);
  });
  ...
  it(`Can't configure if the upcoming funding cycle start timestamp would overflow the max value`, async function () {
    ...
    ).to.be.revertedWith(errors.INVALID_RULESET_END_TIME);
  });
  ...
  it(`Can't configure if ballot is not IERC165 compliant`, async function () {
    const { controller, deployer, mockJbDirectory, jbFundingCycleStore } =
      await setup();
    await mockJbDirectory.mock.controllerOf
      .withArgs(PROJECT_ID)
      .returns(controller.address);

    const mockBallotNew = await deployMockContract(
      deployer,
      ijbFundingCycleBallot.abi
    );
