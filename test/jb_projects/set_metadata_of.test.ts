...
describe('JBProjects::setMetadataOf(...)', function () {
    ...
    it(`Should set MetadataCid on project by owner and emit SetMetadata`, async function () {
        ...
        await jbProjectsStore
            .connect(deployer)
            .createFor(projectOwner.address, { content: METADATA_CID, domain: METADATA_DOMAIN });

        let tx = await jbProjectsStore
            .connect(projectOwner)
            .setMetadataOf(PROJECT_ID_1, { content: METADATA_CID_2, domain: METADATA_DOMAIN_2 });
    ...
    it(`Should set MetadataCid on project if caller is not owner but has permission`, async function () {
        ...
        await expect(jbProjectsStore.connect(addrs[1]).setMetadataOf(PROJECT_ID_1, { content: METADATA_CID_2, domain: 0 })).to
            .not.be.reverted;
    });
