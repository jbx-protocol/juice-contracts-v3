...
describe('JBSplitsPayerDeployer::deploySplitsPayerWithSplits(...)', function () {
  ...
  it(`Should deploy splits payer with splits, correct domain and group, and emit event`, async function () {
    ...
    const expectedGroup = ethers.BigNumber.from(
      ethers.utils.solidityKeccak256(
        ['address', 'uint256'],
        [deployer.address, (await ethers.provider.getBlockNumber()) + 2], // every hardhat transaction mines a new block
      ),
    );
    const expectedGroupedSplit = [{ group: expectedGroup, splits: DEFAULT_SPLITS }];

    await mockJbSplitsStore.mock.set
      .withArgs(DEFAULT_SPLITS_PROJECT_ID, expectedDomain, expectedGroupedSplit)
      .returns();
