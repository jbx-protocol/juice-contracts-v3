    ...
    function testETHAllowance() public {
        ...
        JBFundAccessConstraints[] memory _fundAccessConstraints = new JBFundAccessConstraints[](1);
        {
            ...
            _overflowAllowances[0] =
                JBCurrencyAmount({value: _ethOverflowAllowance, currency: _ethCurrency});
        ...
        uint256 _beneficiaryTokenBalance = PRBMath.mulDiv(_ethPayAmount, _data.weight, 10 ** 18)
            * _metadata.reservedRate / jbLibraries().MAX_RESERVED_RATE();
        ...
        uint256 _beneficiaryBalance = PRBMath.mulDiv(
            _ethOverflowAllowance,
            jbLibraries().MAX_FEE(),
            jbLibraries().MAX_FEE() + _terminal.fee()
        );
        ...
        assertEq(
            ...
            _ethPayAmount - _ethOverflowAllowance
        );

        // Make sure the fee was paid correctly.
        assertEq(
            jbPaymentTerminalStore().balanceOf(
                IJBSingleTokenPaymentTerminal(address(_terminal)), _FEE_PROJECT_ID
            ),
            _ethOverflowAllowance - _beneficiaryBalance
        );
        ...
        assertEq(
            ...
            PRBMath.mulDiv(_ethOverflowAllowance - _beneficiaryBalance, _data.weight, 10 ** 18)
                * _metadata.reservedRate / jbLibraries().MAX_RESERVED_RATE()
        );
        ...
        uint256 _projectOwnerBalance = (_ethDistributionLimit * jbLibraries().MAX_FEE())
            / (_terminal.fee() + jbLibraries().MAX_FEE());
        ...
        assertEq(
            ...
            (_ethOverflowAllowance - _beneficiaryBalance)
                + (_ethDistributionLimit - _projectOwnerBalance)
        );
        assertEq(
            address(_terminal).balance, _ethPayAmount - _beneficiaryBalance - _projectOwnerBalance
        );
        ...
        uint256 _reclaimAmount = (
            PRBMath.mulDiv(
                PRBMath.mulDiv(
                    _ethPayAmount - _ethOverflowAllowance - _ethDistributionLimit,
        ...
        uint256 _feeAmount = _reclaimAmount
            - _reclaimAmount * jbLibraries().MAX_FEE() / (_terminal.fee() + jbLibraries().MAX_FEE());
        ...
        assertEq(
            ...
            (_ethOverflowAllowance - _beneficiaryBalance)
                + (_ethDistributionLimit - _projectOwnerBalance) + _feeAmount
        );
        assertEq(
            address(_terminal).balance,
            _ethPayAmount - _beneficiaryBalance - _projectOwnerBalance
                - (_reclaimAmount - _feeAmount)
        );
        ...
        assertEq(
            ...
            PRBMath.mulDiv(_feeAmount, _data.weight, 10 ** 18) * _metadata.reservedRate
                / jbLibraries().MAX_RESERVED_RATE()
        );
    }

    function testFuzzETHAllowance(
        uint232 _ethOverflowAllowance,
        uint232 _ethDistributionLimit,
        uint256 _ethPayAmount
    ) public {
        // Make sure the amount of eth to pay is bounded.
        _ethPayAmount = bound(_ethPayAmount, 0, 1_000_000 ether);

        // Make sure the values don't overflow the registry.
        unchecked {
            vm.assume(
                _ethOverflowAllowance + _ethDistributionLimit >= _ethOverflowAllowance
                    && _ethOverflowAllowance + _ethDistributionLimit >= _ethDistributionLimit
            );
        }
        ...
        if (_ethOverflowAllowance + _ethDistributionLimit <= _ethPayAmount) {
            // Make sure the beneficiary received the funds and that they are no longer in the terminal.
            _beneficiaryBalance = PRBMath.mulDiv(
                _ethOverflowAllowance,
                jbLibraries().MAX_FEE(),
                jbLibraries().MAX_FEE() + _terminal.fee()
            );
        ...
        if (_ethDistributionLimit <= _ethPayAmount && _ethDistributionLimit != 0) {
            // Make sure the project owner received the distributed funds.
            _projectOwnerBalance = (_ethDistributionLimit * jbLibraries().MAX_FEE())
                / (_terminal.fee() + jbLibraries().MAX_FEE());
            assertEq(_projectOwner.balance, _projectOwnerBalance);
            assertEq(
                jbPaymentTerminalStore().balanceOf(
    ...
    function testFuzzETHAllowanceWithRevertingFeeProject(
        ...
        if (_ethOverflowAllowance + _ethDistributionLimit <= _ethPayAmount) {
            ...
            assertEq(
                ...
                _ethPayAmount - _beneficiaryBalance
            );
