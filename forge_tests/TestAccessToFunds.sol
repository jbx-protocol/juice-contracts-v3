...
contract TestAccessToFunds_Local is TestBaseWorkflow {
    ...
    //     // Make sure the terminal holds the full ETH balance.
    //     assertEq(jbTerminalStore().balanceOf(_terminal, _projectId), _ethPayAmount);
    ...
    //         assertEq(jbTerminalStore().balanceOf(_terminal, _projectId), _ethPayAmount - _beneficiaryBalance);

    //         // Make sure the fee was not taken.
    //         assertEq(jbTerminalStore().balanceOf(_terminal, _FEE_PROJECT_ID), 0);
    //         assertEq(address(_terminal).balance, _ethPayAmount - _beneficiaryBalance);
    ...
    //         assertEq(_projectOwner.balance, _projectOwnerBalance);
    //         // Make sure the fee stays in the treasury.
    //         assertEq(jbTerminalStore().balanceOf(_terminal, _projectId), _ethPayAmount - _beneficiaryBalance - _projectOwnerBalance);
    ...
    //         assertEq(_beneficiary.balance, _beneficiaryBalance + _reclaimAmount - _feeAmount);
    //         // Make sure the fee stays in the treasury.
    //         assertEq(jbTerminalStore().balanceOf(_terminal, _projectId), _ethPayAmount - _beneficiaryBalance - _projectOwnerBalance - (_reclaimAmount - _feeAmount));
